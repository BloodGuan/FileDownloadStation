...
class DirectoryLister {
	...
	public function getListedShortPath() {
    ...
    protected function _setDirectoryPath($dir) {
        ...
        if (!file_exists($dir) || !is_dir($dir)) {
            // Set the error message
            $this->setSystemMessage('danger', '<b>错误:</b> 文件路径不存在');
        ...
        if ($this->_isHidden($dir)) {
            // Set the error message
            $this->setSystemMessage('danger', '<b>错误:</b> 拒绝访问');
        ...
        || strpos($dir, '..') !== false || strpos($dir, '/') === 0) {
            // Set the error message
            $this->setSystemMessage('danger', '<b>错误:</b> 检测到无效的路径字符串');
